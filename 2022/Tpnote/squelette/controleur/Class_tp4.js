var VueTp4 = /** @class */ (function () {
    ...
    VueTp4.prototype.valideSaisie = function () {
        ...
        var nom = this.form.edtNom.value.trim();
        var prenom = this.form.edtPrenom.value.trim();
        var dtnais = this.form.edtDtNais.valueAsDate;
        var experience = this.form.edtExperience.value;
        var diplome = this.form.listeDiplome.value;
        var autre = this.form.edtAutre.value.trim();
        var systeme = "";
        ...
        if (civilite.length === 0) {
            erreur += "Civilité à sélectionner<br>";
        }
        if (nom.length === 0) {
        ...
        if (dtnais === null) {
            erreur += "Date de naissance à renseigner<br>";
        }
        if (diplome.length === 0) {
            erreur += "Diplôme à renseigner<br>";
        }
        var nb = Number(experience); // si la valeur n'est pas un nombre, nb 0
        if (nb < 1) {
            erreur += "Expérience doit être supérieure à 1 année<br>";
        }
        if (systeme.length === 0) {
            erreur += "Au moins un système à cocher<br>";
        }
        else if (!this.form.edtAutre.hidden) {
            // si système "autre" est coché alors "autre" ne doit pas être vide
            if (autre.length === 0) {
                erreur += 'Système "autre" à préciser<br>';
            }
        }
        ...
        if (erreur.length === 0) {
            ...
            var liste = this.form.ListeRecap;
    ...
    VueTp4.prototype.corrigeSaisie = function () {
